function A=coare35vn_modifs_sauvage_etal_2023(u,zu,t,zt,rh,zq,P,ts,Rs,Rl,lat,zi,rain,cp,sigH,theta,use_mean)
%
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% Contact: Cesar Sauvage at csauvage@whoi.edu
% Website: https://cesarsauvage.github.io/
% GitHub: https://github.com/cesarsauvage
%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
...
if use_mean == "yes"
    disp('Using mean wave state');
end

if waveage && seastate && angle
    sigH=sigH(:);cp=cp(:);theta=theta(:);
    disp('Using seastate and wind-wave alignement dependent parameterization');
end

if use_mean == "yes" && angle
    disp('It is not recommended to use both mean wave state and angle with this current version');
    return
end
...
if waveage && ~seastate && ~angle
    cp=cp(:);
    disp('Using wave age dependent parameterization');
end
...
%%%%%%%%% Cesar Sauvage modifications to use mean wave state (Sauvage et al. 2023)
if use_mean == "yes"
    Ad=0.39;  %Mean Sea-state/wave-age dependent coefficients
    Bd=2.6;
end
%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%% Cesar Sauvage modifications to use wind wave (mis)alignement (Sauvage et al. 2023)
%%%%%%%%% This formula is based on findings from previsous study Porchetta et al. 2019
theta1=0.45.*theta;
theta2=-0.32.*theta;

Ad_new=0.091.*cos(theta1);  
Bd_new=2.*cos(theta2);

zoS_new=sigH.*Ad_new.*(usr./cp).^Bd_new;
charnS_new=zoS_new.*grav./usr./usr;
%%%%%%%%%%%%%%%%%%%%
...
for i=1:nits
    ...
    charnS=zoS.*grav./usr./usr;
    %Cesar Sauvage modifications
    zoS_new=sigH.*Ad_new.*(usr./cp).^Bd_new;%-0.11*visa./usr;
    charnS_new=zoS_new.*grav./usr./usr;
...
A=[usr tau hsb hlb hbb hsbb hlwebb tsr qsr zo zoq Cd Ch Ce  L zet dter dqer tkt Urf Trf Qrf RHrf UrfN Rnl Le rhoa UN U10 U10N Cdn_10 Chn_10 Cen_10 RF Qs Evap T10 Q10 RH10 gf rhoa];
