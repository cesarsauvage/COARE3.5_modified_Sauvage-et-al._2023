...
def coare36vn_zrf_et(u, zu , t, zt, rh, zq, P, ts, sw_dn, lw_dn, lat, lon,jd, zi,rain, Ss, cp=None, sigH=None, winddir=None, wavedir=None,use_mean='no', zrf_u=10.0, zrf_t=10.0, zrf_q=10.0):   
#**************************************************************************
# VERSION INFO: Modif Sauvage et al. 2023
...
# Best to avoid NaNs as inputs as well. Will prevent weird results
    ...
    if winddir is not None and winddir.size==1:
        winddir = np.copy(np.asarray([winddir], dtype=float)).flatten()
    elif winddir is None:
        winddir = np.nan * np.ones(N)

    if wavedir is not None and wavedir.size==1:
        wavedir = np.copy(np.asarray([wavedir], dtype=float)).flatten()
    elif wavedir is None:
        wavedir = np.nan * np.ones(N)

    # if use_mean == "yes" and theta is not None:
    #     print('WARNING: It is not recommended to use both mean wave state and theta with this current version')
...
# ii=np.where(isnan(zi))
# zi(ii)=600   # PBL height
# ii=np.where(isnan(Ss))
# Ss(ii)=35    # Salinity
...
# input us = 0*u here;
    ...
    #***********  Calculate angle between wind and wave ***************************
    ii_wind = np.array(np.where(np.logical_not(np.isnan(winddir))))
    ii_wave = np.array(np.where(np.logical_not(np.isnan(wavedir))))
    if ii_wind.size!=0 and ii_wave.size!=0:
        theta_tmp = abs(winddir - wavedir) % 360
        theta = np.where(theta_tmp[:] <= 180,theta_tmp,360 - theta_tmp)

        #convert in radian
    
        theta = theta * (np.pi/180)
    else:
        theta=np.nan * np.ones(N)
    ...
    ######### Cesar Sauvage modifications to use mean wave state (Eq. 12 in Sauvage et al. 2023)
    if use_mean == "yes":
        Ad = 0.39  
        Bd = 2.6
    #########
    ...
    ######### Cesar Sauvage modifications to use wind wave (mis)alignement (Eq. 13 in Sauvage et al. 2023)
    ######### This formula is based on findings from previsous study Porchetta et al. 2019
    if ii_wind.size!=0 and ii_wave.size!=0:
        theta1 = 0.45*theta
        theta2 = -0.32*theta
        
        Ad = 0.2*np.cos(theta1) #0.091
        Bd = 2.2*np.cos(theta2)
    #########
    ...
    out = np.array([usr,tau,hsb,hlb,hbb,hsbb,hlwebb,tsr,qsr,zo,zot,zoq,Cd,Ch,Ce,L,zeta,dT_skinx,dq_skinx,dz_skin,Urf,Trf,Qrf,RHrf,UrfN,TrfN,QrfN,lw_net,sw_net,Le,rhoa,UN,U10,U10N,Cdn_10,Chn_10,Cen_10,hrain,Qs,Evap,T10,T10N,Q10,Q10N,RH10,P10,rhoa10,gust,wc_frac,theta])#Edism])
